...
public partial class IdleTime_Bodyshop : System.Web.UI.Page
{
       ...
       // getGridDetails();
        ...
        if (e.CommandName== "Update_EscalationTime")
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("UpdateTblProcessListProcessTime_BS", con, transaction);
                cmd.CommandType = CommandType.StoredProcedure;
               
                cmd.Parameters.AddWithValue("@ProcessTime", ((TextBox)grvIdelTime.Rows[index].FindControl("txtprocessTime")).Text);
                cmd.Parameters.AddWithValue("@ProcessId", grvIdelTime.Rows[index].Cells[0].Text.ToString().Trim());
                con.Open();
                cmd.ExecuteNonQuery();
                cmd.Parameters.Clear();
                transaction.Commit();
                lbIdleTimeMsg.CssClass = "ScsMsg";
                lbIdleTimeMsg.Text = "Standard time saved.";
    ...
    protected void getGridDetails()
    {
        ...
        SqlCommand cmd = new SqlCommand("UdpGetProcessStandardandEscalationTime_BS", con);
        cmd.CommandType = CommandType.StoredProcedure;
    ...
    protected void grvIdelTime_RowDataBound(object sender, GridViewRowEventArgs e)
    {
