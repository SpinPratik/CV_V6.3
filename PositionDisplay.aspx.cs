...
public partial class PositionDisplay : System.Web.UI.Page
{
    ...
    public static string LoadProcessInOutTime(string RefNo, string ProcessName)
    {
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            {
                str = "<span><table style=font-style:Consolas, Georgia;><tr style=background-color:#c3d9ff;text-align:center;><th style=width:100px;>Employee</th><th style=width:100px;>Employee Type</th><th style=width:100px;>InTime</th><th style=width:100px;>OutTime</th></tr>";
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    str += "<tr style=text-align:center;><td>" + dt.Rows[i]["EmpName"].ToString() + "</td><td>" + dt.Rows[i]["EmployeeType"].ToString() + "</td><td>" + dt.Rows[i]["InTime"].ToString().Replace("#", " ") + "</td><td>" + dt.Rows[i]["OutTime"].ToString() + "</td></tr>";
                }
                str += "</table></span>";
            }
        }
