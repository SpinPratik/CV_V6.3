...
public partial class Baydisplay_TLWise : System.Web.UI.Page
{
    private int pageIndex = 0;
    private DataTable BayColors;
    private string BackColor = "";
    string[] ColumnHeader = new string[100];
    string str = ""; 
    ...
    private void FillBayDisplay()
    {
        ...
        try
        {
            ...
            Array.Clear(ColumnHeader, 0, ColumnHeader.Length);
            SqlConnection con = new SqlConnection(DataManager.ConStr);
            SqlCommand cmd = new SqlCommand();
            cmd.CommandText = "BayProgressDisplay_4Hrs_TeamLeadWise";
            ...
            if (dt.Rows.Count > 0)
            {
                dt.Columns.RemoveAt(dt.Columns.Count - 1);
                 ColumnHeader = dt.Columns.Cast<DataColumn>()
    ...
    protected void timeLine_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            for (int i = 0; i < e.Row.Cells.Count; i++)
            {
                if (i > 3 && i < e.Row.Cells.Count - 3)
                {
                   ...
                   str = e.Row.Cells[i].Text.Substring(0, 6) + " " + e.Row.Cells[i].Text.Substring(6);
                    DateTime datt = DateTime.Parse(str);
                    DataTable ShifData1 = GetShiftDetails();
                    DateTime LunchOut = DateTime.Parse(ShifData1.Rows[0]["BreakOut"].ToString());
                    BreakInData = " " + ShifData1.Rows[0]["BreakIn"].ToString().Split(':').GetValue(0).ToString();
                    ...
                    if (DateTime.Compare(DateTime.Parse(datt.ToString("HH:mm")), DateTime.Parse(DateTime.Now.ToString("HH:mm"))) <= 0)
                    {
                        ...
                        {
                            DateTime LunchIn = DateTime.Parse(ShifData1.Rows[0]["BreakIn"].ToString());
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            //{
            //    if (ColumnHeader[flag].Split(':')[0].ToString() == BreakInData)
            //    {
            //        e.Row.Cells[flag].Visible = false;
            //    }
            //}
            ...
            {
                //if (ColumnHeader[i].Split(':')[0].ToString() == BreakInData)
                //{
                ...
                if (e.Row.Cells[i].Text.Trim() != "" && e.Row.Cells[i].Text.Trim() != "&nbsp;")
                {
                    if (ColumnHeader[i].Split(':')[0].ToString() == BreakInData)
                    {
                        ...
                        e.Row.Cells[i].Attributes.Add("style", "background-color:Red;color:#000000;font-weight:bold;");
                    }
                    else
                    {
                        //e.Row.Cells[i].BackColor = Color.FromName(BayCol);
                        //e.Row.Cells[i].ForeColor = Color.Black;
                    ...
                    if (e.Row.Cells[i].Text.Trim() != "" && e.Row.Cells[i].Text.Trim() != "&nbsp;")
                    {
                        ...
                        if (RegNo.Length > 5)
                        {
                           // e.Row.Cells[i].Text = "<div style=width:1px;><table cellpadding=0 cellspacing=0 style=width:100%;table-layout:fixed;white-space:nowrap;><tr><td align=right valign=middle style=color:black;>" + e.Row.Cells[i].Text + "</td></tr></table></div>";//<tr><td>" + EmpName + "</td></tr>
                            string VehNo = e.Row.Cells[i].Text.Split(' ')[0].ToString();
                            string EmpName = e.Row.Cells[i].Text.Split(' ')[1].ToString();

                            e.Row.Cells[i].Text = "<div style=width:1px;><table cellpadding=0 cellspacing=0 style=width:100%;table-layout:fixed;white-space:nowrap;><tr><td align=right valign=middle style=color:black;>" + VehNo + "</td></tr><tr><td align=right valign=middle style=color:black;>" + EmpName + "</td></tr></table></div>";//<tr><td>" + EmpName + "</td></tr>
                        }
                        else
                        {
                            if (str[4].ToString() == "1")
                            ...
                            {
                                e.Row.Cells[i].Text = "<div style=width:1px;><table cellpadding=0 cellspacing=0 style=width:100%;table-layout:fixed;white-space:nowrap;><tr><td align=right valign=middle style=color:CornflowerBlue;>" + str2 + "</td></tr></table></div>";//<tr><td>" + EmpName + "</td></tr>
                            }
        ...
        if (e.Row.RowIndex > 0)
        {
            if (e.Row.Cells[1].Text.Trim() == timeLine.Rows[e.Row.RowIndex - 1].Cells[1].Text.Trim())
            {
                ...
                timeLine.Rows[e.Row.RowIndex - 1].Cells[1].Text = "<table border=0 cellpadding=0 cellspacing=0><tr><td width=100% align=left style=color:white;font-size:40px>" + e.Row.Cells[1].Text + "</td><td><table border=0 cellpadding=0 cellspacing=0 ><tr><td align=right valign=middle style=color:CornflowerBlue;>P</td></tr><tr><td align=right valign=middle style=color:White;F>A</td></tr></table></td></tr></table>";                
            }
        }
