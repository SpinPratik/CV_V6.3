...
public partial class JobAllotment : System.Web.UI.Page
{
            ...
            try
            {
                if (Request.Form["__EVENTARGUMENT"] != null && Request.Form["__EVENTARGUMENT"].ToString().Trim() != string.Empty)
                {
                    ...
                    try
                    {
                        ...
                        drpJobCode.SelectedIndex = -1;
                        tbJobCode.Visible = true;
    ...
    private void RefreshGrid()
    {
        ...
        try
        {
            ...
            if (ddlEmpType.SelectedItem.Text.ToString() != "--Select--")
                cmd.Parameters.AddWithValue("@EmpType", ddlEmpType.SelectedItem.Text.ToString());
            else
                cmd.Parameters.AddWithValue("@EmpType", "Mechanic");
            cmd.Parameters.AddWithValue("@ShiftId", ddlShift.SelectedValue.ToString());
            if (ddlEmpType.SelectedItem.Text.ToString() == "Mechanic")
            ...
            {
                //|| ddlBay.SelectedValue.ToString() == "Bay"
                if (ddlTeam.SelectedValue.ToString() == "0")
                {
                    timeLine.DataSource = null;
                    timeLine.Visible = false;
                    timeLine.DataBind();
    ...
    private void ClearAllOnce()
    {
        try
        {
            ...
            tbJobCode.Visible = false;
            drpJobCode.Items.Add("JobCode");
            drpJobCode.SelectedIndex = -1;
            btn_AddJobCard.Text = "Add New";
    ...
    {
        SqlCommand cmd1 = new SqlCommand("GetBays_TLWise", con);
        cmd1.CommandType = CommandType.StoredProcedure;
        SqlDataAdapter sda1 = new SqlDataAdapter(cmd1);
        sda1.SelectCommand.Parameters.AddWithValue("@TLId", ddlTeam.SelectedValue.ToString());
        ...
        if (dt1.Rows.Count > 0)
        {
            ...
            ddlBay.DataTextField = "Name";
            ddlBay.DataValueField = "Id";
            ddlBay.DataBind();
        }
    }
    ...
    protected void gdvRegTagList_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        try
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                e.Row.Cells[7].Visible = false;
                e.Row.Cells[8].Visible = false;
                //e.Row.Cells[2].Attributes.Add("onmouseover", "showRegNoHover(event,'" + e.Row.Cells[1].Text.ToString() + "')");
                ...
                //e.Row.Cells[3].Attributes.Add("onmouseover", "showRegNoHover(event,'" + e.Row.Cells[1].Text.ToString() + "')");
    ...
    private static DataTable GetRegNoHover(string RefNo)
    {
        DataTable dt = new DataTable();
        SqlConnection con = new SqlConnection(DataManager.ConStr);
        SqlCommand cmd = new SqlCommand("GetRefNoHoverJobAllotement", con);
        SqlDataAdapter sda = new SqlDataAdapter(cmd);
        cmd.CommandType = CommandType.StoredProcedure;
    ...
    //    {
    //        str = "<table cellpadding='0' cellspacing='0' border='0'><tr style='color: #FFFFFF; background-color: #0066CC;'><td style='padding-left:3px;'><strong>JOB CODES</strong></td></tr>";
    //        if (i % 2 == 0)
    //        {
    //            str = str + "<tr style='height:15px;' bgcolor='#E4E4E4'><td style='padding-left:3px;'>" + dt.Rows[i]["JobCode"].ToString() + "</td></tr>";
    //        }
    //        else
    //        {
    //            str = str + "<tr style='height:15px;' bgcolor='#E4E4E4'><td style='padding-left:3px;'>" + dt.Rows[i]["JobCode"].ToString() + "</td></tr>";
    ...
    protected void gdvRegTagList_SelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            ...
            if (gdvRegTagList.SelectedRow.Cells[6].ForeColor != Color.Orange)
            {
                ...
                lblCurrentKMS.Text = gdvRegTagList.SelectedRow.Cells[9].Text.Trim();
               // lblKMS1.Text = gdvRegTagList.SelectedRow.Cells[9].Text.Trim();
    ...
    protected void tbJobCode_TextChanged(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            cmd.CommandType = CommandType.StoredProcedure;
            if (tbJobCode.Visible == true && tbJobCode.Text.Trim() != "")
                cmd.Parameters.AddWithValue("@JobCode ", tbJobCode.Text.Trim());
            else if (drpJobCode.Visible = true && drpJobCode.Text != "JobCode")
                cmd.Parameters.AddWithValue("@JobCode ", drpJobCode.Text.Trim());
    ...
    protected void btnMechAssign_Click(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            try
            {
                ...
                //if(lblStdTime.Text!="")
                //{
                int allotedMin = Convert.ToInt16(tbAllotTime.Text);
                //int allotedMin = Convert.ToInt16(tbAllotTime.Text) + Convert.ToInt16(lblStdTime.Text);
                int chk = allotedMin % 10;

                if (chk != 0)
                {

                    AllotedTime = allotedMin + (10 - chk);

                }
                else
                {
                    AllotedTime = allotedMin;
                }
                //}
            }
            catch (Exception ex)
            ...
            try
            {
                ...
                cmd.Parameters.AddWithValue("@JobId", lblJobCode.Text);
                cmd.Parameters.AddWithValue("@JobDesc", lblJobDesc.Text);
                cmd.Parameters.AddWithValue("@StdTime", AllotedTime);
                cmd.Parameters.AddWithValue("@AllotTime", AllotedTime);
    ...
    private DataTable GetShiftDetails()
    {
        DataTable dt = new DataTable();
        SqlConnection con = new SqlConnection(DataManager.ConStr);
        SqlCommand cmd = new SqlCommand();
        cmd.CommandText = "udpGetShiftDetails";
        cmd.CommandType = CommandType.StoredProcedure;
        cmd.Connection = con;
        SqlDataAdapter sda = new SqlDataAdapter(cmd);
        sda.Fill(dt);
    ...
    protected void timeLine_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            cell.Text = "<table style=width:80px;><tr><td>Employee</td></tr></table>";
            gvr.Cells.Add(cell);
            for (int i = 5; i < e.Row.Cells.Count - 5; i++)
            {
                ...
                DataTable ShifData = GetShiftDetails();
                BreakInData = ShifData.Rows[0]["BreakIn"].ToString().Split(':').GetValue(0).ToString();
            ...
            for (int i = 0; i < e.Row.Cells.Count; i++)
            {
                if (i > 4 && i < e.Row.Cells.Count - 5)
                {
                    ...
                    DataTable ShifData = GetShiftDetails();
                    BreakInData = " " + ShifData.Rows[0]["BreakIn"].ToString().Split(':').GetValue(0).ToString();
                    DateTime LunchOut = DateTime.Parse(ShifData.Rows[0]["BreakOut"].ToString());
                    ...
                    {
                        if (str.ToString().Split(':').GetValue(0).ToString() == BreakInData)
                        {
                            DateTime LunchIn = DateTime.Parse(ShifData.Rows[0]["BreakIn"].ToString());
                            while (LunchIn < LunchOut)
                            {
                                if (DateTime.Compare(DateTime.Parse(datt.ToString("HH:mm")), DateTime.Parse(LunchIn.ToString("HH:mm"))) == 0)
                                {
    ...
    protected void cmbTechName_SelectedIndexChanged(object sender, EventArgs e)
    {
        RefreshGrid();
        RefreshGridforBay();
        if (drpJobCode.Visible == true)
        {
            if (lblRefNo.Text.Trim() == "")
            {
                lblMessage.Text = "Please Select Vehicle From the List.";
            }
            else if (cmbTechName.Text.ToString() == "--Select--")
            {
                lblMessage.Text = "Please Select Technician.";
            ...
            {
                //tbJobCode_AutoCompleteExtender.ContextKey = lblRefNo.Text.ToString() + "," + cmbTechName.SelectedValue.ToString();
                GetJobCodes(drpJobCode);
            }
        }
    }
    protected void GetJobCodes(DropDownList ddl)
    {
        try
        {
            ddl.Items.Clear();
            ...
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("@RefId", lblRefNo.Text.ToString());
            cmd.Parameters.AddWithValue("@TechId", cmbTechName.SelectedValue.ToString());
            SqlDataAdapter sda = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            if (con.State != ConnectionState.Open)
                con.Open();
    ...
    protected void timeLine_Bay_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.Header)
        {
            ...
            TableCell cell = new TableCell();
            cell.HorizontalAlign = HorizontalAlign.Center;
            cell.Text = "<table style=width:70px;><tr><td>BAY</td></tr></table>";
            gvr.Cells.Add(cell);
    ...
    protected void btn_AddJobCard_Click(object sender, EventArgs e)
    {
        RefreshGrid();
        RefreshGridforBay();
        if (tbJobCode.Visible = true && btn_AddJobCard.Text == "Job List")
        {
            drpJobCode.Visible = true;
            tbJobCode.Visible = false;
            btn_AddJobCard.Text = "Add New";
        ...
        else
        {
            tbJobCode.Visible = true;
            drpJobCode.Visible = false;
            btn_AddJobCard.Text = "Job List";

        }

    }
    protected void tbAllotDate_TextChanged(object sender, EventArgs e)
    {
       // ClearAll();
        //// getRegTag();
        RefreshGrid();
    ...
    protected void ImageButton1_Click(object sender, ImageClickEventArgs e)
    {
        ...
        string JobCode = "";
        string JobDesc = "";
        if (btnTechUpdate.Visible == false)
        {
            if (tbAllotTime.Text != "" && lblStdTime.Text != "")
            {
                if (tbAllotTime.Text != lblStdTime.Text)
                {
                    JobCode = lblJobCode.Text.ToString();
                    JobDesc = tbJobDesc.Text.ToString();
                    TotalAllotedMin = Convert.ToInt16(tbAllotTime.Text.ToString());
                    JobCode = JobCode + "," + tbJobCode.Text.ToString();
                    JobDesc = JobDesc + "," + tbJobDesc.Text;
                    lblJobCode.Text = "";
                    lblJobDesc.Text = "";
                    lblJobCode.Text = JobCode;
                    lblJobDesc.Text = JobDesc;
                    TotalAllotedMin = TotalAllotedMin + Convert.ToInt16(lblStdTime.Text.ToString());
                    tbAllotTime.Text = TotalAllotedMin.ToString();
                    //JobCode.Add(tbJobCode.Text.ToString());
                    //JobDesc.Add(tbJobDesc.Text);
                }
                else
                {
                    TotalAllotedMin = Convert.ToInt16(tbAllotTime.Text.ToString());
                    lblJobCode.Text = tbJobCode.Text.ToString();
                    lblJobDesc.Text = tbJobDesc.Text.ToString();
                    //JobCode.Add(tbJobCode.Text.ToString());
                    //JobDesc.Add(tbJobDesc.Text);
                }
            }
            ...
            {
                lblMessage.Text = "Please enter JobCode..!";
            }
            tbJobCode.Text = "";
            tbJobDesc.Text = "";
            lblStdTime.Text = "";
